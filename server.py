...
def index():
    if "chats" not in session:
        session['chats'] = [('eliza', "Hello. How are you feeling today?")]
    session['chats'] = session.get("chats", [])
    
    return render_template("index.html", chats=session['chats'])

@app.route("/chat", methods=["POST"])
def input():
    oldchat = session.get("chats", [])
    ...
    oldchat.append(("user", request.form['input']))
    oldchat.append(("eliza", eliza_response))

    session['chats'] = oldchat
    # print session['chats']
    return redirect('/')
